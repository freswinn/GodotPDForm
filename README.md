...
![](/addons/GodotPDForm/icon.png)

In this fork, I have modified the code somewhat, mostly adding documentation and typecasting, but also adding new classes of nodes to the Godot editor. With these classes, you are now able to build a scene that acts as a form for your PDF document, which you can then directly call to export as a PDF.

Check out the wiki! [[https://github.com/freswinn/GodotPDForm/wiki]]
...
## PDFormBase
The base of these nodes is `PDFormBase`. While `PDFormBase` is required to use the **PDForm nodes**, it is not at all necessary to create a scene where the root node is a `PDFormBase` -- just recommended.

In the inspector for `PDFormBase`, you can set your document's title and author, select which page is currently visible, and list the file paths of any and all fonts you want to import into the document when exporting.
...
# Exporting the PDF
Once you have your scene saved, you merely need to call the `build_export(filepath)` function from the `PDFormBase` node! The filepath needs to be the absolute path, including the filename and `.pdf` extension.
